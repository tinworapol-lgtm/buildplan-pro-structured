...
module.exports = async function handler(request, response) {
  ...
  const rows = await supabaseRest('subscriptions?user_id=eq.' + encodeURIComponent(session.user.id) + '&select=status,plan,current_period_end,stripe_customer_id&order=current_period_end.desc&limit=1');
